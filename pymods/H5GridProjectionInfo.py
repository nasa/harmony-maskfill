...
"""
def get_hdf_proj4(h5_dataset, shortname):
    objname = h5_dataset.name

    grid_mapping = get_grid_mapping_data(get_grid_mapping_group(shortname, objname))
    if not grid_mapping:
...
def get_short_name(h5_dataset):
    return CFConfig.getShortName(h5_dataset.file)

def get_grid_mapping_group(shortname, objname):
    return CFConfig.getGridMappingGroup(shortname,objname)

def get_grid_mapping_data(projection):
...
"""
def get_lat_lon_datasets(h5_dataset):
...
""" Gets the cell height and width of the gridded HDF5 dataset from the dataset's latitude and longitude coordinate datasets.
    Note: the cell height is expected to be negative because the row indices of image data increase downwards.
    ...
    Returns:
        tuple: cell width, cell height
"""
def get_cell_size_from_lat_lon(h5_dataset):
...
""" Finds the min and max locations in both coordinate axes of the dataset. 
    ...
    Returns:
        tuple: x min, x max, y min, y max
"""
def get_corner_points_from_lat_lon(h5_dataset):
    lat, lon = get_lat_lon_datasets(h5_dataset)

    shortname = get_short_name(h5_dataset)
    proj4_str = get_grid_mapping_data(get_grid_mapping_group(shortname, h5_dataset.name))
...
""" Gets the lat/lon arrays of the HDF5 dataset.
    ...
    Returns:
        tuple: The x coordinate array and the y coordinate array
"""
def get_lat_lon_arrays(h5_dataset):
