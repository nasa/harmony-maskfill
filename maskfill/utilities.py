...
def get_mask_array(
    ...
    """
    bounded_shape_gdf = get_bounded_shape(shape_path, crs, out_shape, transform)
    ...
    if transform.e == 0.0:
        transform = Affine(
            transform.a, transform.b, transform.c, transform.d, transform.a, transform.f
        )
    # Column width is zero, assign row height.
    elif transform.a == 0.0:
        transform = Affine(
            transform.e, transform.b, transform.c, transform.d, transform.e, transform.f
        )
...
def get_bounded_shape(
    ...
    if epsg is not None and not should_ignore_pyproj_bounds(crs):
        ...
        minx, miny, maxx, maxy = get_grid_geographic_bounds(transform, crs, out_shape)
    ...
    if crs.is_geographic:
        ...
        shape_gdf = get_resolved_dataframe(shape_path, transform, crs, out_shape)
...
def mask_fill_array(
    raster_arr: np.ndarray, mask_array: np.ndarray, fill_value: float
) -> np.ndarray:
...
def mask_fill_dataset_by_chunks(
    h5_dataset: Dataset, mask_array: np.ndarray, fill_value: float
) -> None:
    ...
    if h5_dataset.chunks is not None:
        ...
        chunk_slices_iterator = [tuple(slice(0, extent) for extent in h5_dataset.shape)]

    for chunk_slices in chunk_slices_iterator:
        h5_dataset[chunk_slices] = mask_fill_array(
            h5_dataset[chunk_slices], mask_array[chunk_slices], fill_value
        )
...
def get_h5_mask_array_id(h5_dataset: Dataset, crs: CRS, shape_path: str) -> str:
...
def get_transform_information(h5_dataset: Dataset) -> str:
    ...
    if get_dimension_datasets(h5_dataset):
        ...
        dimension_names = ', '.join(
            [h5_file[reference[0]].name for reference in dimension_list]
        )
...
def apply_2d_dataset_to_multidim(
    h5_dataset: Dataset, cf_config: CFConfig, process, *args
):
...
def get_resolved_dataframe(
    ...
    """
    geographic_resolution = get_grid_geographic_resolution(transform, crs, out_shape)
...
def get_geographic_resolution(longitudes: np.ndarray, latitudes: np.ndarray) -> float:
    ...
    if latitudes.shape[0] == 1:  # one row
        ...
        lon_square_diffs = np.square(
            np.subtract(longitudes[1:, 1:], longitudes[:-1, :-1])
        )
        lat_square_diffs = np.square(
            np.subtract(latitudes[1:, 1:], latitudes[:-1, :-1])
        )
    else:
        return 0
...
def get_grid_geographic_bounds(
    ...
    perimeter_columns = np.concatenate(
        [column_indices, column_indices, np.zeros(rows), np.full(rows, columns - 1)]
    )
    perimeter_rows = np.concatenate(
        [np.zeros(columns), np.full(columns, rows - 1), row_indices, row_indices]
    )
    ...
    latitudes, longitudes = to_geo_transformer.transform(projected_x, projected_y)
...
def get_grid_geographic_resolution(
    ...
    to_geo_transformer = Transformer.from_crs(crs, 4326)
    projected_x_row = transform.a * np.arange(columns, dtype='float64') + transform.c
    ...
    for block_start in range(0, rows, block_rows):
        ...
        block_latitudes, block_longitudes = to_geo_transformer.transform(
            np.broadcast_to(projected_x_row, (block_stop - block_start, columns)),
            np.broadcast_to(
                projected_y[:, np.newaxis], (block_stop - block_start, columns)
            ),
        )
        ...
        if block_latitudes.shape[0] > 1:
            resolution = min(
                resolution, get_geographic_resolution(block_longitudes, block_latitudes)
            )
...
def get_resolved_shape(
    geo_dataframe: gpd.GeoDataFrame, resolution: float
) -> gpd.GeoDataFrame:
    ...
    """
    polygons = [
        get_resolved_polygon(feature, resolution)
...
def get_resolved_polygon(feature: dict, resolution: float) -> Polygon:
    ...
    exterior_ring = get_resolved_ring(list(feature_shape.exterior.coords), resolution)

    interior_rings = [
        get_resolved_ring(list(interior.coords), resolution)
...
def get_resolved_ring(
    ...
    """
    new_points = [
        get_resolved_line(point_one, ring_points[point_one_index + 1], resolution)[:-1]
        for point_one_index, point_one in enumerate(ring_points[:-1])
    ]
...
def raise_maskfill_exception(exception: Exception):
    ...
    raise ExceptionClass('Maskfill failed with error: ' + str(exception)) from exception
