...
def get_hdf_crs(h5_dataset: Dataset, cf_config: CFConfig, logger: Logger) -> CRS:
    ...
    * Finally, check the global MaskFill configuration file for default
    ...
    config_grid_mapping = cf_config.get_dataset_grid_mapping_attributes(h5_dataset.name)

    if has_geographic_dimensions(h5_dataset):
        logger.debug(
            f'Dataset {h5_dataset.name} has dimensions with '
            'units "degrees". Using Geographic coordinates.'
        )
        crs = CRS.from_epsg(4326)
    elif grid_mapping_name is not None:
        logger.debug(
            f'Dataset {h5_dataset.name} has grid_mapping data, '
        ...
        logger.debug(
            f'Dataset {h5_dataset.name} has no projection '
...
def has_geographic_dimensions(h5_dataset: Dataset) -> bool:
    ...
    """
    units_list = [
        get_decoded_attribute(dimension, 'units', default='')
        for dimension in get_dimension_datasets(h5_dataset) or ()
    ]
...
def get_lon_lat_datasets(
    h5_dataset: Dataset, cf_config: CFConfig
    ...
    coordinate_list = cf_config.get_coordinate_overrides(h5_dataset.name) or re.split(
        '[, ]', get_decoded_attribute(h5_dataset, 'coordinates', '')
    )
    latitude = None
    longitude = None
    for coordinate in coordinate_list:
        try:
            qualified_coordinate = resolve_relative_dataset_path(h5_dataset, coordinate)
...
def get_dimension_datasets(h5_dataset: Dataset) -> tuple[Dataset, Dataset] | None:
    ...
    if any(
        dim is None or all(value == 0.0 for value in dim)
        for dim in (column_dimension, row_dimension)
    ):
        return None
...
def is_projection_x_dimension(dimension_dataset: Dataset) -> bool:
    ...
    return get_decoded_attribute(dimension_dataset, 'units') in longitude_units or (
        get_decoded_attribute(dimension_dataset, 'standard_name')
...
def is_projection_y_dimension(dimension_dataset: Dataset) -> bool:
    ...
    return get_decoded_attribute(dimension_dataset, 'units') in latitude_units or (
        get_decoded_attribute(dimension_dataset, 'standard_name')
...
def get_dataset_attributes(h5_dataset: Dataset) -> dict:
    ...
    """
    return {
        attribute_key: get_decoded_attribute(h5_dataset, attribute_key)
        for attribute_key in h5_dataset.attrs
    }
...
def get_transform(
    h5_dataset: Dataset, crs: CRS, cf_config: CFConfig, logger: Logger
) -> Affine:
    ...
    if get_dimension_datasets(h5_dataset):
        ...
        x_0, x_n, y_0, y_m = get_corner_points_from_lat_lon(
            h5_dataset, crs, cf_config, logger
        )
        cell_width, cell_height = get_cell_size_from_lat_lon_extents(
            h5_dataset, x_0, x_n, y_0, y_m, cf_config
        )
...
def get_cell_size_from_lat_lon_extents(
    h5_dataset: Dataset,
    x_0: float,
    x_n: float,
    y_0: float,
...
def get_corner_points_from_lat_lon(
    h5_dataset: Dataset, crs: CRS, cf_config: CFConfig, logger: Logger
) -> CornerPoints:
    """Finds the min and max locations in both coordinate axes of the dataset.
    Args:
    ...
    if len(lon.shape) >= 3:
        ...
        logger.debug(
            f'lat/lon for {h5_dataset.name} is 3-D, using first band for coordinates.'
        )
        band = 0
    else:
        band = None
    ...
    if band is not None:
        ...
        lat_values = lat[band]
        lon_corner_values = [
            lon_values[lower_left_tuple],
            lon_values[upper_right_tuple],
        ]
        lat_corner_values = [
            lat_values[lower_left_tuple],
            lat_values[upper_right_tuple],
        ]
    else:
        lon_values = lon[:]
        lat_values = lat[:]
    ...
    if lon_fill_value in lon_corner_values or lat_fill_value in lat_corner_values:
        ...
        valid_indices = np.where(
            (lon_values != lon_fill_value) & (lat_values != lat_fill_value)
        )

        # Get the extent in each direction, projected x and projected y:
        x_0, x_n = get_projected_coordinate_extent(
            projection, lat_values, lon_values, 1, valid_indices
        )
        y_0, y_m = get_projected_coordinate_extent(
            projection, lat_values, lon_values, 0, valid_indices
        )
    else:
...
def get_projected_coordinate_extent(
    projection: Proj,
    latitude: np.ndarray,
    ...
    longitude_min = longitude[
        valid_indices[0][argmin_valid_index], valid_indices[1][argmin_valid_index]
    ]
    longitude_max = longitude[
        valid_indices[0][argmax_valid_index], valid_indices[1][argmax_valid_index]
    ]
    ...
    if argmax_valid_index != argmin_valid_index:
        ...
        pixel_size = (max_point[projection_dim] - min_point[projection_dim]) / (
            index_of_max_point - index_of_min_point
        )
    else:
        raise InsufficientDataError(
            f'Only a single, unmasked {line_type} '
...
def get_fill_value(
    ...
    if config_fill_value is not None:
        logger.debug(
            f'The dataset {h5_dataset.name} has a known incorrect fill '
...
def dataset_all_fill_value(
    ...
    if dataset.chunks is not None:
        return all(
            np.all(dataset[chunk_slices] == fill_value)
...
def resolve_relative_dataset_path(h5_dataset: Dataset, relative_path: str) -> str:
