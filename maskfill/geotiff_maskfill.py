...
def produce_masked_geotiff(
    ...
    exclusions = [
        convert_variable_path(exclusion_path)
    ...
    if variable_should_be_masked(geotiff_path, exclusions):
        ...
        logger.debug('Successfully created masked GeoTIFF and updated metadata')
    else:
        # Copy input file to output path
        logger.debug(
            f'{geotiff_path} matches coordinate exclusion rule; '
...
def get_mask_array(
    geotiff_path: str, shape_path: str, cache_dir: str, mask_grid_cache: str
) -> np.ndarray:
    ...
    mask_array = get_geotiff_cached_mask_array(
        geotiff_path, shape_path, cache_dir, mask_grid_cache
    )

    if mask_array is None:
        mask_array = create_mask_array(geotiff_path, shape_path)
        cache_geotiff_mask_array(
            mask_array, geotiff_path, shape_path, cache_dir, mask_grid_cache
        )
...
def get_fill_value(
    ...
    if fill_value is None and default_fill_value is not None:
        logger.info(
            'The GeoTIFF does not have a fill value, so the default '
            f'fill value {default_fill_value} will be used'
        )
        fill_value = default_fill_value
    elif fill_value is None:
        logger.info(
            'GeoTIFF does not have fill value, and not default '
