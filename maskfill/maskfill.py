...
def mask_fill(input_file: str, shape_file: str, output_dir: str,
              mask_grid_cache: str, fill_value: float | None,
              logger: logging.Logger, bounding_box=None) -> str:
    """ Performs a mask fill on the given data file using the supplied shape
        file, applying the fill value to all pixels outside the shapes and
        writing the masked output to `output_dir`.
