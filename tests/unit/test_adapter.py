...
class TestHarmonyMaskFill(TestCase):
        ...
        cls.message = Message(
            {
                ...
                'subset': {
                    'shape': {'href': cls.shape_usa, 'type': 'application/geo+json'}
                },
                'user': 'cyeaget',
            }
        )
        cls.input_stac = create_input_stac(cls.input_hdf5, 'application/x-hdf5')
        cls.harmony_adapter = MaskFillAdapter(
            cls.message, config=config(False), catalog=cls.input_stac
        )
        basic_log_config(
    ...
    def test_validate_input_granule(self):
        ...
        with self.subTest('Inferred MIME type of None fails validation'):
            ...
            self.assertEqual(
                context.exception.message, 'Invalid granule format: .unknown'
            )
        ...
        with self.subTest('Unexpected MIME type fails validation'):
            ...
            self.assertEqual(
                context.exception.message, 'Invalid granule format: application/json'
            )
    ...
    def test_get_file_mimetype(self):
        ...
        for description, file_name, expected_mimetype in test_arguments:
            with self.subTest(description):
                self.assertEqual(
                    self.harmony_adapter.get_file_mimetype(file_name), expected_mimetype
                )

    @patch('maskfill.adapter.download')
    def test_download_from_remote(self, mock_download):
        ...
        def download_side_effect(url, directory, logger, access_token, cfg) -> str:
    ...
    def test_message_has_valid_shape_file(self):
        ...
        with self.subTest('Undefined Message.subset.shape.href'):
            message = Message({'subset': {'shape': {'type': 'application/geo+json'}}})
            harmony_adapter = MaskFillAdapter(
            ...
            self.assertEqual(
                str(context_manager.exception), 'Shape file must specify resource URL.'
            )
        ...
        with self.subTest('Missing shape file MIME type'):
            ...
            self.assertEqual(
                context_manager.exception.message, 'Shape file must be GeoJSON format.'
            )
