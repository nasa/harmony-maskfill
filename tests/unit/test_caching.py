...
class TestCaching(TestCase):
    ...
    def test_cache_geotiff_mask_array(
        self, mock_np_save, mock_get_geotiff_mask_array_id
    ):
    ...
    def test_cache_h5_mask_arrays(self, mock_np_save):
        ...
        """
        h5_masks = {
            'mask_id_1': np.array([[1, 2], [3, 4]]),
            'mask_id_2': np.array([[5, 6], [7, 8]]),
            'mask_id_3': np.array([[9, 0], [1, 2]]),
        }

        with self.subTest('Should save'):
            cache_h5_mask_arrays(h5_masks, self.cache_dir, 'use_and_save', self.logger)

            self.assertEqual(mock_np_save.call_count, 3)
            mock_np_save.assert_any_call(
                f'{self.cache_dir}/mask_id_1.npy', h5_masks['mask_id_1']
            )
            mock_np_save.assert_any_call(
                f'{self.cache_dir}/mask_id_2.npy', h5_masks['mask_id_2']
            )
            mock_np_save.assert_any_call(
                f'{self.cache_dir}/mask_id_3.npy', h5_masks['mask_id_3']
            )
        ...
        with self.subTest('Does not save; "delete" in grid caching option'):
            cache_h5_mask_arrays(
                h5_masks, self.cache_dir, 'ignore_and_delete', self.logger
            )
            mock_np_save.assert_not_called()
    ...
    def test_get_geotiff_cached_mask_array(
        self, mock_np_load, mock_os_exists, mock_get_geotiff_mask_array_id
    ):
        ...
        with self.subTest('"use" in the grid caching option returns data'):
            ...
            mask_array = get_geotiff_cached_mask_array(
                self.tif_data_path, self.shape_path, self.cache_dir, 'use_cache'
            )
        ...
        with self.subTest('"use" not in grid caching option, returns `None`'):
            mock_os_exists.return_value = True
            mask_array = get_geotiff_cached_mask_array(
                self.tif_data_path, self.shape_path, self.cache_dir, 'ignore_and_delete'
            )
            self.assertEqual(mask_array, None)
    ...
    def test_get_geotiff_mask_array_path(self, mock_get_geotiff_mask_array_id):
        ...
        mask_array_path = get_geotiff_mask_array_path(
            self.tif_data_path, self.shape_path, self.cache_dir
        )
