...
class TestUtilities(TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.exterior_ring = [(3, 5), (1, 4), (1, 2), (5, 1), (6, 2), (6, 4), (3, 5)]
        cls.interior_ring = [(3, 4), (4, 3), (2, 3), (3, 4)]
        cls.resolution = 2
        cls.resolved_exterior_ring = [
            (3, 5),
            (2, 4.5),
            (1, 4),
            (1, 2),
        ...
        cls.hollow_polygon = Polygon(cls.exterior_ring, holes=[cls.interior_ring])
        cls.resolved_solid_polygon = Polygon(cls.resolved_exterior_ring)
        cls.resolved_hollow_polygon = Polygon(
            cls.resolved_exterior_ring, holes=[cls.interior_ring]
        )
        ...
        for point_index, point_one in enumerate(list_one):
            for coordinate_index, coordinate in enumerate(point_one):
                self.assertAlmostEqual(
                    coordinate, list_two[point_index][coordinate_index], places=3
                )
    ...
    def test_get_h5_mask_array_id(self):
        ...
        with self.subTest('DIMENSION_LIST absent, different coordinates'):
            expected_mask_id_one = (
                '1f689a7016523c01a6436da39fba978f26d73ee4200fbd3a0ccd613e'
            )
            expected_mask_id_two = (
                '95aa4fb28d44ae780450d85e362385a762ed917d48dc8ccf6e4ae293'
            )
    ...
    def test_get_bounded_shape(self):
        ...
        with self.subTest('Creating bounded shape from CRS with EPSG code'):
            ...
            bounded_gdf = get_bounded_shape(
                shape_path, self.ease_two_north, out_shape, transform
            )
        ...
        with self.subTest('UTM projections do not use pyproj bounds'):
            ...
            with patch('maskfill.utilities.CRS') as mock_crs:
                bounded_gdf = get_bounded_shape(
                    'tests/data/COL.geo.json', utm_crs, out_shape, transform
                )
    ...
    def test_create_bounding_box_shape_file(self):
        ...
        expected_coordinates = [[[10, 20], [30, 20], [30, 40], [10, 40], [10, 20]]]
        geojson_file_path = create_bounding_box_shape_file(bounding_box, self.tmp_dir)
        ...
        self.assertEqual(actual_geojson['features'][0]['geometry']['type'], 'Polygon')
        ...
        self.assertTupleEqual(
            parsed_dataframe['geometry'][0].bounds, tuple(bounding_box)
        )
    ...
    def test_get_transform_information(self):
        ...
        with self.subTest('DIMENSION_LIST absent'):
            ...
            expected_result = (
                f'coords: {group}/latitude.Bands_01 {group}/longitude.Bands_01'
            )
            transform = get_transform_information(dataset)
    ...
    def test_get_resolved_ring(self):
        ...
        """
        test_args = [
            ['Exterior ring', self.exterior_ring, self.resolved_exterior_ring],
            [
                'Interior ring (no extra points)',
                self.interior_ring,
        ...
        for description, input_ring, expected_output in test_args:
            with self.subTest(description):
                self.assert_points_equal(
                    get_resolved_ring(input_ring, self.resolution), expected_output
                )
    ...
    def test_get_resolved_polygon(self):
        ...
        """
        test_args = [
            ['Solid polygon', self.solid_polygon, self.resolved_solid_polygon],
            ['Polygon with hole', self.hollow_polygon, self.resolved_hollow_polygon],
        ]

        for description, input_polygon, expected_output in test_args:
            with self.subTest(description):
                gdf = GeoDataFrame(crs='epsg:4326', geometry=[input_polygon])
                resolved_output = get_resolved_polygon(
                    next(gdf.iterfeatures()), self.resolution
                )

                self.assert_points_equal(
                    list(resolved_output.exterior.coords),
    ...
    def test_get_resolved_shape_multipolygon(self):
        ...
        second_polygon_points = [
            (point[0] + 10, point[1] + 10) for point in self.exterior_ring
        ]
        second_polygon_input = Polygon(second_polygon_points)

        expected_second_output_points = [
            (point[0] + 10, point[1] + 10) for point in self.resolved_exterior_ring
        ]
        ...
        input_gdf = GeoDataFrame(
            crs='epsg:4326', geometry=[self.solid_polygon, second_polygon_input]
        )
    ...
    def test_get_resolved_dataframe(self):
        ...
        output_dataframe = get_resolved_dataframe(
            shape_path, transform, self.ease_two_north, out_shape
        )
        ...
        assert_geodataframe_equal(
            output_dataframe, expected_dataframe, check_less_precise=True
        )
    ...
    def test_get_grid_lat_lons(self):
        ...
        expected_latitudes = np.array(
            [[10.0, 9.979, 9.959], [10.116, 10.096, 10.075], [10.232, 10.212, 10.191]]
        )
        expected_longitudes = np.array(
            [[10.0, 10.069, 10.138], [10.012, 10.081, 10.150], [10.024, 10.093, 10.162]]
        )

        latitudes, longitudes = get_grid_lat_lons(
    ...
    def test_get_grid_geographic_bounds_matches_full_grid(self):
        ...
        expected_bounds = (
            np.nanmin(longitudes),
            np.nanmin(latitudes),
            np.nanmax(longitudes),
            np.nanmax(latitudes),
        )

        self.assertTupleEqual(
            get_grid_geographic_bounds(transform, self.ease_two_north, out_shape),
            expected_bounds,
        )
    ...
    def test_get_grid_geographic_resolution_matches_full_grid(self):
        ...
        for block_rows in [1, 7, 25, 256]:
            with self.subTest(f'block_rows = {block_rows}'):
                self.assertEqual(
                    get_grid_geographic_resolution(
                        transform, self.ease_two_north, out_shape, block_rows=block_rows
                    ),
                    expected_resolution,
                )
    ...
    def test_mask_fill_dataset_by_chunks(self):
        ...
        test_args = [
            ['Chunked dataset with partial edge chunks', (3, 2)],
        ...
        for description, chunks in test_args:
            with self.subTest(description):
                ...
                with h5py.File(h5_path, 'w') as h5_file:
                    dataset = h5_file.create_dataset('data', data=data, chunks=chunks)
                    mask_fill_dataset_by_chunks(dataset, mask_array, fill_value)
    ...
    def test_get_decoded_attribute(self):
        ...
        default_test_args = [
            [
                'Default not used when value present',
                'string_value',
                'default',
                string_value,
            ],
        ...
        with h5py.File('test.h5', 'w', driver='core', backing_store=False) as h5_file:
            ...
            for description, attribute_key, expected_value in test_args:
                with self.subTest(description):
                    self.assertEqual(
                        get_decoded_attribute(h5_file, attribute_key), expected_value
                    )

            for description, key, default, expected_value in default_test_args:
                with self.subTest(description):
                    self.assertEqual(
                        get_decoded_attribute(h5_file, key, default), expected_value
                    )
    ...
    def test_get_default_fill_for_data_type(self):
        ...
        with self.subTest('float64 returns -9999.0'):
            self.assertEqual(get_default_fill_for_data_type('float64'), -9999.0)

        with self.subTest('uint8 returns 254'):
            self.assertEqual(get_default_fill_for_data_type('uint8'), 254)
        with self.subTest('None returns -9999.0'):
            self.assertEqual(get_default_fill_for_data_type(None), -9999.0)
        with self.subTest('Unrecognised type returns -9999.0'):
            self.assertEqual(
                get_default_fill_for_data_type('random_type_string'), -9999.0
            )
    ...
    def test_apply_2d_methods(self):
        ...
        with self.subTest('apply_2d to nominal shaped granule'):
            ...
            expected_masked_output = np.array(
                [
                    [
                        [-9999, 1, -9999],
                        [-9999, 4, -9999],
            ...
            np.testing.assert_array_equal(
                apply_2d(array_2x4x3, cf_config, mask_fill_array, mask_4x3, -9999),
    ...
    def test_apply_2d_dataset_to_multidim(self, mock_get_spatial_grid_shape):
        ...
        with self.subTest('apply on 3d dataset'):
            mask_array = np.array([[True, False, True], [True, False, True]])
            expected_masked_output = np.array(
                [
                    [
                        [-9999, 1, -9999],
            ...
            result = apply_2d_dataset_to_multidim(
                dset_3d, cf_config, mask_fill_array, mask_array, -9999
            )

            np.testing.assert_array_equal(result, expected_masked_output)

        with self.subTest('apply on 4d dataset'):
            mask_array = np.array([[False, True, False], [False, True, False]])
            expected_masked_output = np.array(
                [
                    [
                        [[-0, -9999, 2], [3, -9999, 5], [6, -9999, 8], [9, -9999, 11]],
                        [
                            [12, -9999, 14],
                            [15, -9999, 17],
                            [18, -9999, 20],
            ...
            result = apply_2d_dataset_to_multidim(
                dset_4d, cf_config, mask_fill_array, mask_array, -9999
            )
