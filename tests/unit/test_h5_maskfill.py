...
class TestH5MaskFill(MaskFillTestCase):
    ...
    def test_mask_fill_no_processing(self, mock_get_mask_array):
        ...
        with self.subTest('Variable name exactly matches an exclusion'):
            dataset = h5_file.create_dataset(
                '/excluded/variable', data=np.ones((3, 2)), fillvalue=0
            )
        ...
        with self.subTest('Dataset coordinates contain only fill values'):
            ...
            dataset = h5_file.create_dataset(
                'variable', data=np.ones((3, 2)), fillvalue=dataset_fill_value
            )
            latitude_ds = h5_file.create_dataset(
                'latitude', data=np.ones((3, 2)), fillvalue=latitude_fill_value
            )
            longitude_ds = h5_file.create_dataset(
                'longitude', data=np.zeros((3, 2)), fillvalue=latitude_fill_value
            )
            dataset.attrs['coordinates'] = '/latitude, /longitude'
    ...
    @patch('maskfill.h5_maskfill.get_mask_array')
    def test_mask_fill_write_paths(
        self, mock_get_mask_array, mock_get_apply_2d_process
    ):
        ...
        mask_array = np.array(
            [[1, 0, 1], [1, 0, 1], [1, 0, 1], [1, 0, 1]], dtype=np.uint8
        )
        ...
        with self.subTest('Observed statistics retain the whole-array path'):
            dataset = h5_file.create_dataset(
                'with_statistics', data=data, chunks=(3, 2)
            )
            dataset.attrs['_FillValue'] = fill_value
            dataset.attrs['coordinates'] = '/latitude /longitude'
            ...
            self.assertEqual(dataset.attrs['observed_max'], np.max(expected_unfilled))
            self.assertEqual(dataset.attrs['observed_min'], np.min(expected_unfilled))
            self.assertEqual(dataset.attrs['observed_mean'], np.mean(expected_unfilled))
    ...
    def test_get_mask_array(self, mock_create_mask_array):
        ...
        with self.subTest('No prior mask (cached or saved)'):
            ...
            mock_create_mask_array.assert_called_once_with(
                dataset, crs, self.shape_file, self.cf_config, self.logger
            )
    ...
    def test_get_string_variables(self):
        ...
        with h5py.File(self.sample_nc4_file(), 'r+') as input_file:
            ...
            input_file.create_dataset('fixed_ascii_s10', dtype='S10')

            # 2. Fixed-length ASCII (alternative syntax)
            input_file.create_dataset(
                'fixed_ascii_np', data=ascii_data, dtype=np.dtype('S15')
            )

            # 3. Variable-length Unicode strings
            vlen_str = h5py.special_dtype(vlen=str)
            input_file.create_dataset('vlen_unicode', data=unicode_data, dtype=vlen_str)

            # 4. Variable-length Unicode with UTF-8 encoding
            utf8_variable = h5py.string_dtype(encoding='utf-8')
            input_file.create_dataset(
                'variable_utf8', data=mixed_length_data, dtype=utf8_variable
            )
    ...
    def test_create_mask_array(self):
        ...
        with self.subTest('Multiple Row Col subset'):
            ...
            dataset = h5_file[
                '/Soil_Moisture_Retrieval_Data_1km/surface_temperature_1km'
            ]
            expected_mask = np.zeros((48, 53))

            mask_array = create_mask_array(
                dataset,
                crs,
        ...
        with self.subTest('Subset has one row, multiple columns'):
            ...
            mask_array = create_mask_array(
                dataset, crs, 'tests/data/one_row.geojson', self.cf_config, self.logger
            )
            np.testing.assert_array_equal(mask_array, expected_mask)
            h5_file.close()
        with self.subTest('Subset has one column, multiple rows'):
            ...
            mask_array = create_mask_array(
                dataset, crs, 'tests/data/one_col.geojson', self.cf_config, self.logger
            )
            np.testing.assert_array_equal(mask_array, expected_mask)
            h5_file.close()
